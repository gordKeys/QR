...
class FTMOComplianceEngine:
    """Execution-layer FTMO guard; it does not alter strategy signals or sizing."""
    ...
    def __init__(self, config: FTMOComplianceConfig):
        ...
        self.balance = None
        self.equity = None
        self.prague_now = None
        self.daily_base = None
    ...
    def refresh_account(self, *, balance, equity, now_utc=None):
        ...
        if self.state.get("prague_day") != current_day:
            ...
            self._save_state()
        daily_base = float(self.state.get("daily_base") or self.balance)
        highest_base = float(self.state.get("highest_daily_base") or daily_base)
        ...
        self.daily_limit = daily_base - daily_loss
    ...
    def platform_market_closed(self, now_utc=None):
        ...
        if now.weekday() == 4 and (now.hour > 23 or (now.hour == 23 and now.minute >= 60 - buffer)):
            return True
