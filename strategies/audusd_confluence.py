...
class AUDUSDConfluenceStrategy(BaseStrategy):
    ...
    def _add_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        frame["atr"] = tr.ewm(alpha=1 / 14, min_periods=14, adjust=False).mean()
    ...
    def _trend_direction(self, broker, symbol):
        ...
        timeframe = self.trend_timeframe or broker.mt5.TIMEFRAME_M15
        rates = broker.rates_copy(symbol, timeframe, 70)
    ...
    def generate_signals(self, data: pd.DataFrame):
        ...
        for index in range(25, len(frame)):
            ...
            buy = {
                ...
                "rsi": current["rsi"] <= self.oversold,
                "bb": current["close"] <= current["bb_lower"] * 1.001,
                "candle": self._bull_engulf(current, previous) or self._bull_pin(current),
            }
            sell = {
                "ema": previous["ema_fast"] >= previous["ema_slow"] and current["ema_fast"] < current["ema_slow"],
                "macd": current["macd_h"] < 0 and current["macd_h"] < previous["macd_h"],
                "rsi": current["rsi"] >= self.overbought,
                "bb": current["close"] >= current["bb_upper"] * 0.999,
    ...
    def analyze_trade(self, data: pd.DataFrame, *, broker=None, symbol=None, equity=0.0, signal=None, price=None):
        ...
        return {
            ...
            "target": target,
            "size": volume,
            "size_reason": "audusd_confluence_dry_run",
        }
